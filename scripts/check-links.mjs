...
for (const f of htmlFiles) {
  for (const { file, href } of extractHrefs(f)) {
    ...
    if (href.startsWith('#')) {
      ...
      if (file === 'index.html' && !allIds.has(id)) {
...
const assets = [
  ...
  'favicon.svg', 'sitemap.xml', 'llms.txt', 'og-image.png', 'og-image.svg'
];
...
const pages = ['/', '/privacy.html', '/404.html', '/assets/site.js', '/favicon.svg', '/llms.txt', '/sitemap.xml', '/og-image.png'];
const httpResults = [];
for (const p of pages) {
  const r = await get(base, p);
  httpResults.push(r);
  if (r.status !== 200) issues.push({ file: 'server', href: p, problem: `HTTP ${r.status}` });
}
